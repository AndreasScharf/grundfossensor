...
class grundfossensor(object):
    ...
    def __init__(self, serial, barcode, sensor_id, type, my_os='L', debug=False):
        self.ports = []
        self.use_port = ''
        self.my_os = my_os
        if my_os == 'L':
    ...
    def init(self, barcode, sensor_id):
        ...
        for port in self.ports:
          ...
          if anwser == 'Error' or anwser == 'Timeout':
            continue
          else:
            self.error = False
            if self.debug:
    ...
    def change_baudrate(self, baudrate):
      b_id = [1200, 4800, 9600, 19200,
              56000 if not self.type == 'MFS' else 57600, 11520]
